...
public class TestDefaultRecordBlobProvider {
    ...
    public void testWriteErrors() throws Exception {
        ...
        // must have a doc id
        try {
            bp.writeBlob(blob, "", FILE_CONTENT);
            fail();
        } catch (NuxeoException e) {
    ...
    public void testCRUD() throws Exception {
        ...
        // write a blob
        String key = bp.writeBlob(bbi.blob, ID, FILE_CONTENT);
        assertEquals(ID, key);
        ...
        // update the blob
        BlobAndBlobInfo bbi2 = BlobAndBlobInfo.of("lorem ipsum", MIME_TYPE, "UTF-8", "lorem.txt");
        String key2 = bp.writeBlob(bbi2.blob, ID, FILE_CONTENT);
        ...
        try {
            ...
            assertEquals("Nonexistent file for key: 12345", e.getMessage());
        }
    }
    ...
    public void testTransactionSeveralBlobs() throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            Blob blob = new StringBlob(HELLO_WORLD, MIME_TYPE, ENCODING, FILENAME);
            String key = bp.writeBlob(blob, "docid" + i, FILE_CONTENT);
    ...
    protected void blobWriter(int i, CyclicBarrier barrier, List<Throwable> exc) {
        TransactionHelper.runInTransaction(() -> {
            try {
                ...
                String key = bp.writeBlob(blob, ID, FILE_CONTENT);
                assertEquals(ID, key);
    ...
    protected void blobDeleter(int i, String id, CyclicBarrier barrier, List<Throwable> exc) {
        TransactionHelper.runInTransaction(() -> {
            try {
                barrier.await();
                // delete blob
    ...
    public void testTransactionConcurrencyCreateCreate() throws Throwable {
        ...
        assertEquals("One thread should throw an exception", 1, exc.size());
